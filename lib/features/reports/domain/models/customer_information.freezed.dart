// coverage:ignore-file
// GENERATED CODE - DO NOT MODIFY BY HAND
// ignore_for_file: type=lint
// ignore_for_file: unused_element, deprecated_member_use, deprecated_member_use_from_same_package, use_function_type_syntax_for_parameters, unnecessary_const, avoid_init_to_null, invalid_override_different_default_values_named, prefer_expression_function_bodies, annotate_overrides, invalid_annotation_target, unnecessary_question_mark

part of 'customer_information.dart';

// **************************************************************************
// FreezedGenerator
// **************************************************************************

T _$identity<T>(T value) => value;

final _privateConstructorUsedError = UnsupportedError(
  'It seems like you constructed your class using `MyClass._()`. This constructor is only meant to be used by freezed and you are not supposed to need it nor use it.\nPlease check the documentation here for more information: https://github.com/rrousselGit/freezed#adding-getters-and-methods-to-our-models',
);

CustomerInformation _$CustomerInformationFromJson(Map<String, dynamic> json) {
  return _CustomerInformation.fromJson(json);
}

/// @nodoc
mixin _$CustomerInformation {
  String get customerName => throw _privateConstructorUsedError;
  String get phone => throw _privateConstructorUsedError;
  String get email => throw _privateConstructorUsedError;
  String get address => throw _privateConstructorUsedError;

  /// Serializes this CustomerInformation to a JSON map.
  Map<String, dynamic> toJson() => throw _privateConstructorUsedError;

  /// Create a copy of CustomerInformation
  /// with the given fields replaced by the non-null parameter values.
  @JsonKey(includeFromJson: false, includeToJson: false)
  $CustomerInformationCopyWith<CustomerInformation> get copyWith =>
      throw _privateConstructorUsedError;
}

/// @nodoc
abstract class $CustomerInformationCopyWith<$Res> {
  factory $CustomerInformationCopyWith(
    CustomerInformation value,
    $Res Function(CustomerInformation) then,
  ) = _$CustomerInformationCopyWithImpl<$Res, CustomerInformation>;
  @useResult
  $Res call({String customerName, String phone, String email, String address});
}

/// @nodoc
class _$CustomerInformationCopyWithImpl<$Res, $Val extends CustomerInformation>
    implements $CustomerInformationCopyWith<$Res> {
  _$CustomerInformationCopyWithImpl(this._value, this._then);

  // ignore: unused_field
  final $Val _value;
  // ignore: unused_field
  final $Res Function($Val) _then;

  /// Create a copy of CustomerInformation
  /// with the given fields replaced by the non-null parameter values.
  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? customerName = null,
    Object? phone = null,
    Object? email = null,
    Object? address = null,
  }) {
    return _then(
      _value.copyWith(
            customerName:
                null == customerName
                    ? _value.customerName
                    : customerName // ignore: cast_nullable_to_non_nullable
                        as String,
            phone:
                null == phone
                    ? _value.phone
                    : phone // ignore: cast_nullable_to_non_nullable
                        as String,
            email:
                null == email
                    ? _value.email
                    : email // ignore: cast_nullable_to_non_nullable
                        as String,
            address:
                null == address
                    ? _value.address
                    : address // ignore: cast_nullable_to_non_nullable
                        as String,
          )
          as $Val,
    );
  }
}

/// @nodoc
abstract class _$$CustomerInformationImplCopyWith<$Res>
    implements $CustomerInformationCopyWith<$Res> {
  factory _$$CustomerInformationImplCopyWith(
    _$CustomerInformationImpl value,
    $Res Function(_$CustomerInformationImpl) then,
  ) = __$$CustomerInformationImplCopyWithImpl<$Res>;
  @override
  @useResult
  $Res call({String customerName, String phone, String email, String address});
}

/// @nodoc
class __$$CustomerInformationImplCopyWithImpl<$Res>
    extends _$CustomerInformationCopyWithImpl<$Res, _$CustomerInformationImpl>
    implements _$$CustomerInformationImplCopyWith<$Res> {
  __$$CustomerInformationImplCopyWithImpl(
    _$CustomerInformationImpl _value,
    $Res Function(_$CustomerInformationImpl) _then,
  ) : super(_value, _then);

  /// Create a copy of CustomerInformation
  /// with the given fields replaced by the non-null parameter values.
  @pragma('vm:prefer-inline')
  @override
  $Res call({
    Object? customerName = null,
    Object? phone = null,
    Object? email = null,
    Object? address = null,
  }) {
    return _then(
      _$CustomerInformationImpl(
        customerName:
            null == customerName
                ? _value.customerName
                : customerName // ignore: cast_nullable_to_non_nullable
                    as String,
        phone:
            null == phone
                ? _value.phone
                : phone // ignore: cast_nullable_to_non_nullable
                    as String,
        email:
            null == email
                ? _value.email
                : email // ignore: cast_nullable_to_non_nullable
                    as String,
        address:
            null == address
                ? _value.address
                : address // ignore: cast_nullable_to_non_nullable
                    as String,
      ),
    );
  }
}

/// @nodoc
@JsonSerializable()
class _$CustomerInformationImpl implements _CustomerInformation {
  const _$CustomerInformationImpl({
    required this.customerName,
    this.phone = '',
    this.email = '',
    this.address = '',
  });

  factory _$CustomerInformationImpl.fromJson(Map<String, dynamic> json) =>
      _$$CustomerInformationImplFromJson(json);

  @override
  final String customerName;
  @override
  @JsonKey()
  final String phone;
  @override
  @JsonKey()
  final String email;
  @override
  @JsonKey()
  final String address;

  @override
  String toString() {
    return 'CustomerInformation(customerName: $customerName, phone: $phone, email: $email, address: $address)';
  }

  @override
  bool operator ==(Object other) {
    return identical(this, other) ||
        (other.runtimeType == runtimeType &&
            other is _$CustomerInformationImpl &&
            (identical(other.customerName, customerName) ||
                other.customerName == customerName) &&
            (identical(other.phone, phone) || other.phone == phone) &&
            (identical(other.email, email) || other.email == email) &&
            (identical(other.address, address) || other.address == address));
  }

  @JsonKey(includeFromJson: false, includeToJson: false)
  @override
  int get hashCode =>
      Object.hash(runtimeType, customerName, phone, email, address);

  /// Create a copy of CustomerInformation
  /// with the given fields replaced by the non-null parameter values.
  @JsonKey(includeFromJson: false, includeToJson: false)
  @override
  @pragma('vm:prefer-inline')
  _$$CustomerInformationImplCopyWith<_$CustomerInformationImpl> get copyWith =>
      __$$CustomerInformationImplCopyWithImpl<_$CustomerInformationImpl>(
        this,
        _$identity,
      );

  @override
  Map<String, dynamic> toJson() {
    return _$$CustomerInformationImplToJson(this);
  }
}

abstract class _CustomerInformation implements CustomerInformation {
  const factory _CustomerInformation({
    required final String customerName,
    final String phone,
    final String email,
    final String address,
  }) = _$CustomerInformationImpl;

  factory _CustomerInformation.fromJson(Map<String, dynamic> json) =
      _$CustomerInformationImpl.fromJson;

  @override
  String get customerName;
  @override
  String get phone;
  @override
  String get email;
  @override
  String get address;

  /// Create a copy of CustomerInformation
  /// with the given fields replaced by the non-null parameter values.
  @override
  @JsonKey(includeFromJson: false, includeToJson: false)
  _$$CustomerInformationImplCopyWith<_$CustomerInformationImpl> get copyWith =>
      throw _privateConstructorUsedError;
}
